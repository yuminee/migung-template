...
async def problem2(request: Request, user_answer: str = Form(...)):
...
async def problem3(request: Request, user_answer: str = Form(...)):
    if user_answer == '초록색':
...
async def problem4(request: Request, user_answer: str = Form(...)):
...
async def problem5(request: Request, user_answer: str = Form(...)):
    if user_answer == 'open':
...
async def problem6(request: Request, user_answer: str = Form(...)):
    if user_answer == 'holloween@asleep.ai':
        return templates.TemplateResponse("problem5.html", {"request": request})
    else:
        return templates.TemplateResponse("problem4.html", {"request": request})

@app.post('/problem/bbb795627678d1e76c31869ba885283f')
async def problem7(request: Request, user_answer: str = Form(...)):
    if user_answer == '전화벨':
...
async def purpleA(request: Request, user_answer: str = Form(...)):
    if user_answer=='초':
...
async def purpleB(request: Request, user_answer: str = Form(...)):
    if user_answer=='거울':
...
async def purpleC(request: Request, user_answer: str = Form(...)):
    if user_answer=='낫':
...
async def purpleD(request: Request, user_answer: str = Form(...)):
    if user_answer=='닭':
...
async def purpleE(request: Request, user_answer: str = Form(...)):
    if user_answer=='해골':
        return templates.TemplateResponse("purpleE.html", {"request": request})
    else:
        return templates.TemplateResponse("purpleD.html", {"request": request})

@app.get('/problem/green')
...
async def greenA(request: Request, user_answer: str = Form(...)):
    if user_answer=='초':
...
async def greenB(request: Request, user_answer: str = Form(...)):
    if user_answer=='깃털':
...
async def greenC(request: Request, user_answer: str = Form(...)):
    if user_answer=='가마솥':
...
async def greenD(request: Request, user_answer: str = Form(...)):
    if user_answer=='쥐':
...
async def greenE(request: Request, user_answer: str = Form(...)):
    if user_answer=='독약':
...
async def redA(request: Request, user_answer: str = Form(...)):
    if user_answer=='초':
...
async def redB(request: Request, user_answer: str = Form(...)):
    if user_answer=='옥춘당':
...
async def redC(request: Request, user_answer: str = Form(...)):
    if user_answer=='부채':
...
async def redD(request: Request, user_answer: str = Form(...)):
    if user_answer=='돼지':
...
async def redE(request: Request, user_answer: str = Form(...)):
    if user_answer=='무당방울':
        return templates.TemplateResponse("redE.html", {"request": request})
    else:
        return templates.TemplateResponse("redD.html", {"request": request})

@app.get('/problem/blue')
...
async def blueA(request: Request, user_answer: str = Form(...)):
    if user_answer=='초':
...
async def blueB(request: Request, user_answer: str = Form(...)):
    if user_answer=='호박':
...
async def blueC(request: Request, user_answer: str = Form(...)):
    if user_answer=='칼':
...
async def blueD(request: Request, user_answer: str = Form(...)):
    if user_answer=='박쥐':
...
async def blueE(request: Request, user_answer: str = Form(...)):
    if user_answer=='액자':
